#!/usr/bin/env python3
"""Download and verify the PCM-NO P-NS dataset release assets."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download the P-NS train/validation/test files and verify SHA-256 hashes."
    )
    parser.add_argument(
        "--repo",
        default=None,
    ...
    parser.add_argument(
        "--base-url",
        default=None,
        help="Direct base URL containing the release assets; overrides --repo and --tag.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def read_checksums(path: Path) -> dict[str, str]:
    ...
    missing = [name for name in FILES if name not in checksums]
    if missing:
        raise RuntimeError(
            f"{path} has no checksum entries for {missing}. Run prepare_data_assets.py first."
        )
    return checksums
...
def main() -> int:
    ...
    print(f"P-NS dataset is ready in: {args.output_dir.resolve()}")
    return 0
